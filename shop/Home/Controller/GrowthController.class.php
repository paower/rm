	...
	public function upgrade()
	{
		...
		if($uplevel == 1){
			...
			if($plevel<$uplevel){
				$indent['loudan'][] = $pid;
				for ($i=0; $i < 1; $i++) { 
					$pid = M('user')->where("userid = $pid")->getField('pid');
					$plevel = M('user')->where("userid = $pid")->getField('userlevel');
					if($plevel<$uplevel){
						$i--;
					}else{
						$indent['order1'] = M('user')->where("userid = $pid")->find();
					}
				}
			}else{
				$indent['order1'] = M('user')->where("userid = $pid")->find();
			}
			//第二个订单--根据用户准备升级的级别派单给客户的上几代--等级不满足要求的继续往上找--升一级时找上级为16级的用户 其他级别按代数查找
			$pid = M('user')->where("userid = $id")->getField('pid');
			$plevel = M('user')->where("userid = $pid")->getField('userlevel');
			if($pid!=0){
				for ($i=0; $i < 15; $i++) {
					...
					$plevel = M('user')->where("userid = $pid")->getField('userlevel');
					if($pid==0){
						$pid = M('user')->where("pid = 0")->getField('userid');
						$plevel = M('user')->where("userid = $pid")->getField('userlevel');
					}
				}
				if($plevel<$uplevel){
					...
					for ($i=0; $i < 1; $i++) { 
						...
						if($plevel<$uplevel){
							...
							$indent['order2'] = M('user')->where("userid = $pid")->find();
						}
					}
				}else{
					$indent['order2'] = M('user')->where("userid = $pid")->find();
				...
				$sonid = M('user')->where("pid = 0")->getField('userid');
				$indent['order2'] = M('user')->where("userid = $sonid")->find();
			}

			//第三个订单--根据用户等级--派管理奖订单--0~4级找等级为5级的第一个上级--5~9级找等级为10的第一个上级--10~15找等级为16的第一个上级 假如下面有高等级的管理奖级别 管理奖会被截断
			$pid = M('user')->where("userid = $id")->getField('pid');
			$plevel = M('user')->where("userid = $pid")->getField('userlevel');
			if($plevel!=5 && $plevel!=10 && $plevel!=16){
				for ($i=0; $i < 1; $i++) { 
					$pid = M('user')->where("userid = $pid")->getField('pid');
					$plevel = M('user')->where("userid = $pid")->getField('userlevel');
					if($plevel!=5 && $plevel!=10 && $plevel!=16){
						...
						$indent['order3'] = M('user')->where("userid = $pid")->find();
					}
				}
			}else{
				$indent['order3'] = M('user')->where("userid = $pid")->find();
		...
		if($uplevel == 5){
			//判断是否满足升级条件
			//条件一
			$zhitui = M('user')->where("pid = $id")->select();
		...
		//		$sonid = [];
		//		$sonid[] = M('user')->where("pid = $v")->select(); 
		...
		//		}
		//	}
			// dump($condition1);dump($condition2);
			if($condition1<1){
				$this->error('未满足升级条件');
			}
			//第一个订单--满足等级的上级订单
		...
		if($b==0){
			...
			$order1['up'] = $uplevel;
			$order1['shouid'] = $indent['order1']['userid']; 
			$order1['money'] = $uplevel*5;
			...
			$order2['up'] = $uplevel;
			$order2['shouid'] = $indent['order2']['userid']; 
			$order2['money'] = $uplevel*10;
			...
			//第三张订单
			$order3['uid']=session('userid');
			$order3['up'] = $uplevel;
			$order3['shouid'] = $indent['order3']['userid']; 
			$order3['money'] = $uplevel*5;
			$order3['id']=chr(rand(65,90)).chr(rand(65,90)).rand(100000,999999);
			$order3['type'] = 3;
			//检查升级表是否存在该id 如果存在 重新生成
			$where['id'] = $order3['id'];
			$a = M('upgrade')->where($where)->count();
